...
import { isDemoModeEnabled, DEMO_USER } from "@/hooks/useDemoMode";
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (demoMode) {
      // In demo mode, set a fake user immediately
  ...
  const signUp = async (email: string, password: string, clinicName?: string) => {
    ...
    // Update profile with clinic name after signup if provided
    if (!error && clinicName) {
      // Profile is created by trigger, we'll update it
      setTimeout(async () => {
