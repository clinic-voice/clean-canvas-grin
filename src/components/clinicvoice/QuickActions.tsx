    ...
    <div className="rounded-xl bg-card border border-border overflow-hidden">
      ...
      <div className="p-4 grid grid-cols-2 gap-3">
        {actions.map((action) => (
          <button
            key={action.label}
            className="flex flex-col items-center gap-2 p-4 rounded-xl bg-muted/30 border border-border hover:border-cv-primary/50 hover:bg-cv-primary/5 transition-all"
          >
            <action.icon className={`w-6 h-6 ${action.color}`} />
            <span className="text-xs text-cv-text-secondary font-medium">{action.label}</span>
          </button>
        ))}
      </div>
    </div>
